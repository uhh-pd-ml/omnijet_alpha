...
#SBATCH --output /beegfs/desy/user/birkjosc/gabbro_output/logs/slurm_logs/%x_%j.log      # ADJUST THIS to your log path
...
LOGFILE="/beegfs/desy/user/birkjosc/gabbro_output/logs/slurm_logs/${SLURM_JOB_NAME}_${SLURM_JOB_ID}.log"  # ADJUST THIS to your log path
...
singularity exec --nv --bind /beegfs:/beegfs \
    --env SLURM_JOB_ID="$SLURM_JOB_ID" --env SLURM_LOGFILE="$LOGFILE" \
    /beegfs/desy/user/birkjosc/singularity_images/omnijet-latest.sif \
